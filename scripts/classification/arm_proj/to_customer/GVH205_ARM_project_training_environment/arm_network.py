...
def arm_network_v4_5(ctx):
    network = nn.HybridSequential()
    network.add(
        nn.Conv2D(channels=20, kernel_size=3, padding=1), # conv1
        nn.BatchNorm(axis=1, center=True, scale=True),
        nn.Activation(activation='relu'),
        nn.MaxPool2D(pool_size=2, strides=2),

        nn.Conv2D(channels=40, kernel_size=3, padding=1), # conv2
        nn.BatchNorm(axis=1, center=True, scale=True),
        nn.Activation(activation='relu'),
        nn.MaxPool2D(pool_size=2, strides=2),

        nn.Conv2D(channels=80, kernel_size=3, padding=1), # conv3
        nn.BatchNorm(axis=1, center=True, scale=True),
        nn.Activation(activation='relu'),
        nn.MaxPool2D(pool_size=2, strides=2),

        nn.Conv2D(channels=160, kernel_size=3, padding=1), # conv4
        nn.BatchNorm(axis=1, center=True, scale=True),
        nn.Activation(activation='relu'),
        nn.MaxPool2D(pool_size=2, strides=2),
