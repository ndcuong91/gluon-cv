...
def main():
    ...
    kwargs = {'ctx': context, 'pretrained': opt.use_pretrained, 'classes': 1000}
    if opt.use_gn:
    ...
    if opt.use_rec:
        ...
        train_data, val_data, batch_fn = get_data_loader(batch_size, num_workers)
    ...
    def train(ctx):
        ...
        if save_frequency and save_dir:
            net.save_parameters(os.path.join(save_dir,date_time,'%s-%s-%d.params'%(dataset, model_name, opt.num_epochs-1)))
            trainer.save_states(os.path.join(save_dir,date_time,'%s-%s-%d.states'%(dataset, model_name, opt.num_epochs-1)))
