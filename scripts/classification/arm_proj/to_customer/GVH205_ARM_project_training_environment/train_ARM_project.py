...
transform_train = transforms.Compose([
    transforms.RandomResizedCrop(int(1.0 *opts.input_sz)),
...
def train(train_path, test_path):
    ...
    print 'Begin training', model_name,'with input size:', opts.input_sz,"and dataset:",dataset
    ...
    for epoch in range(opts.resume_epoch, opts.num_epochs):
        ...
        if val_acc_top1 > best_acc:
            ...
            trainer.save_states(os.path.join(folder, date_time, '%s-best-%d.states' % (model_name, epoch)))
        ...
        if opts.save_frequency and epoch % opts.save_frequency == 0 and epoch>0:
            ...
            trainer.save_states(os.path.join(folder, date_time, '%s-%d.states' % (model_name, epoch)))
