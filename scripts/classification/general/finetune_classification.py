...
def parse_args():
    ...
    parser.add_argument('--rec-train', type=str, default=rec_train,
                        help='the training data')
    parser.add_argument('--rec-train-idx', type=str, default=rec_train_idx,
                        help='the index of training data')
    parser.add_argument('--rec-val', type=str, default=rec_val,
                        help='the validation data')
    parser.add_argument('--rec-val-idx', type=str, default=rec_val_idx,
                        help='the index of validation data')
    parser.add_argument('--use-rec', default=use_rec,
    ...
    parser.add_argument('--use-pretrained', default=use_pretrained,
...
def get_network(model_name, opt, ctx):
    ...
    network = get_model(model_name, **kwargs)
    with network.name_scope():
        network.output = nn.Dense(classes)
        network.output.initialize(mx.init.Xavier(), ctx=ctx)
        network.collect_params().reset_ctx(ctx)
        #network.hybridize()
    network.cast(opt.dtype)

    if opt.resume_params is not '':
        network.load_parameters(opt.resume_params, ctx = ctx)
...
def main():
    opt = parse_args()
    save_dir = opt.model+'_'+str(opt.input_size)
    date_time = datetime.now().strftime('%Y-%m-%d_%H.%M')

    logger=setup_logger(os.path.join(save_dir,date_time,'train_'+dataset+'.log'))
    logger.info(opt)
    ...
    net=get_network(model_name,opt,context)
    ...
    def train(ctx):
        ...
        for epoch in range(opt.resume_epoch, opt.num_epochs):
            ...
            if err_top1_val < best_val_score:
                ...
                net.save_parameters(os.path.join(save_dir, date_time, '%s-%s-best.params' % (dataset, model_name)))
                trainer.save_states(os.path.join(save_dir, date_time, '%s-%s-best.states' % (dataset, model_name)))

            if save_frequency and save_dir and (epoch + 1) % save_frequency == 0:
                net.save_parameters(os.path.join(save_dir, date_time, '%s-%s-%d.params' % (dataset, model_name, epoch)))
                trainer.save_states(os.path.join(save_dir, date_time, '%s-%s-%d.states' % (dataset, model_name, epoch)))

        if save_frequency and save_dir:
            net.save_parameters(
                os.path.join(save_dir, date_time, '%s-%s-%d.params' % (dataset, model_name, opt.num_epochs - 1)))
            trainer.save_states(
                os.path.join(save_dir, date_time, '%s-%s-%d.states' % (dataset, model_name, opt.num_epochs - 1)))
